...
### Fixed  
- **2025-08-15**: Admin auth "Access token required" popup (comprehensive fix) — Root cause: grower-dashboard.tsx using lib/api.ts bypassed fetchJson authentication (traced via request IDs). Fixed by routing all apiRequest() calls through fetchJson utility. Added comprehensive tripwire test to prevent regression. All admin API calls now 100% authenticated.
