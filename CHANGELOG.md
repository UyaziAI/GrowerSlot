...
### Fixed  
- **2025-08-15**: Unauthenticated admin calls (pinpointed) — Request ID ib4csz1ucmedcsdz6 traced to grower-dashboard.tsx useQuery race condition (lines 22-25). Enhanced http.ts logging with caller context + gating snapshots. Fixed with isAuthReady gating on useQuery calls. Added ultimate tripwire test for 100% authentication coverage.
- **2025-08-15**: DayEditor runtime error + admin auth noise — Fixed "isOpen is not defined" error in DayEditorSheet.tsx:83 by adding missing isOpen prop to AdminPage.tsx call. Enhanced query gating with isOpen && dateISO validation. Eliminated runtime errors and associated unauthenticated calls when sheet is closed.
- **2025-08-15**: Admin recurring errors (comprehensive resolution) — Structured logging identified 2 recurring patterns: GET /v1/slots 401 every 30s (useSlotsRange polling) + POST /v1/restrictions/apply 401. Root cause: TWO API clients, api/client.ts bypassed fetchJson. Fixed by routing api/client.ts through global auth. Added pattern-specific tripwire tests.
